...
const Model = () => {
  const gltf = useLoader(GLTFLoader, '/3dmodels/gaming_desktop_pc/scene.gltf');
  const modelRef = useRef();

  // Increase the size of the model by adjusting the scale
  const modelScale = [0.1, 0.1, 0.1]; // Adjust the values as needed
